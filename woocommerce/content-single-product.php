...
<div <?php echo $product_schema; ?> id="product-<?php the_ID(); ?>" <?php post_class( $classes ); ?>>
	...
	<div class="section-product-spec">
		<?php
			if ( get_field('is_table_spec') ) {

				$tableSpec = '';
				$tableSpec = get_field('table_spec');
				if($tableSpec !='') {
					echo '<div>' . $tableSpec . '</div>';
				}
			}
		?>
		
		
<!-- 		Display Product Spec Image -->	
		<?php
			
			if(get_field('enable_product_spec_image')) {
				
				$productSpecImg = '';
				$productSpecImg = get_field('product_spec_image');
				if ($productSpecImg !='') {
					echo '<div class="product-spec-img">';
					echo '<h4>คุณสมบัติ</h4>';
					echo '<img src="' . $productSpecImg . '">';
					echo '</div>';
				}
			}
		?>
	...
	<div class="cooking-bg">
		<img src="<?php echo get_template_directory_uri(); ?>/images/cooking-bg-left1.png" class="cooking-bg-left">
		<img src="<?php echo get_template_directory_uri(); ?>/images/cooking-bg-right1.png" class="cooking-bg-right">
		<?php
			
			if(get_field('enable_product_spec_features')){
				$productFeatures = '';
				$productFeatures = get_field('product_spec_features');
				if($productFeatures !='') {
					echo '<div class="features-container">' . $productFeatures . '</div>';
				}
			}
		?>
		...
		<?php
			...
			if(get_field('enable_product_warranty')) {
				$productWarranty = '';
				$productWarranty = get_field('product_warranty');
				if($productWarranty !='') {
					echo '<div class="warranty-container">' . $productWarranty . '</div>';
				}
			}
		?>
